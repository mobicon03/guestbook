...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="mobicon03/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/mobicon03/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/mobicon03/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://mobicon03:${GITHUB_ACCESS_TOKEN}@github.com/mobicon03/guestbook-config.git"
                    '''
                }
            }
